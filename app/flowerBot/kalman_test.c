  ...
  while (1) {
    kobukiSensorPoll(&sensors);
		switch(state) {
          ...
          state = DRIVING;
					...
					display_write("OFF: KALMAN", DISPLAY_LINE_0);
    ...
    case ORIENT_COUNTERCLOCKWISE: {
      ...
      if (is_button_pressed(&sensors)) {
        lsm9ds1_stop_gyro_integration();
        state = OFF;
      } else if (getDistanceDifferenceKalman(&frontDist, pinTrigFront, pinEchoFront, &rightDist, pinTrigRight, pinEchoRight, 5) < difference_tolerance) {
        lsm9ds1_stop_gyro_integration();
        state = DRIVING;
      } else if (fabs(angle) >= orientation_turning_max) {
        lsm9ds1_stop_gyro_integration();
